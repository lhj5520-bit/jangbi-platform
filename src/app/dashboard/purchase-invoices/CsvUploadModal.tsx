...
const HEADER_MAP: Record<string, keyof Row> = {
  // 일반
  '중기업체': 'supplier_name', '중기업체명': 'supplier_name', '업체명': 'supplier_name',
  ...
  // 홈택스 매입 세금계산서 - 첫 번째 '상호'가 공급자(업체)
  '상호': 'supplier_name',
  '공급자상호': 'supplier_name', '공급자 상호': 'supplier_name',
  '작성일자': 'issue_date', '발급일': 'issue_date', '발급일자': 'issue_date', '품목일자': 'issue_date',
  '세액': 'vat_amount',
  '합계금액': 'total_amount', '합계 금액': 'total_amount',
  // 사업자번호 / 주민번호 모두 business_no로
  '공급자사업자등록번호': 'business_no',
  '공급자주민등록번호': 'business_no',
  '사업자등록번호': 'business_no',
  '주민등록번호': 'business_no',
  '대표자명': 'representative',
  '비고': 'memo',
...
export default function PurchaseCsvUploadModal({ onClose, onSaved }: Props) {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // 헤더 행 찾기
    const KNOWN = ['상호', '합계금액', '공급가액', '세액', '작성일자', '발급일자', '공급자']
    let headerIdx = 0
    ...
    // 중복 헤더 처리 (홈택스: '상호'·'사업자등록번호' 두 번 등장)
  ...
  function applyMap() {
    const rows: Row[] = rawRows.map(row => {
      ...
      return {
        supplier_name: '', project_name: '', issue_date: '', period_start: '', period_end: '',
        supply_amount: '', vat_amount: '', total_amount: '', business_no: '', representative: '', memo: '',
        ...r,
      }
    }).filter(r => r.supplier_name && r.supply_amount)
    setPreview(rows)
    setStep('preview')
  }
  ...
  async function handleImport() {
    ...
    const supplierMap = Object.fromEntries((suppliers ?? []).map((s: any) => [s.name, s.id]))
    ...
    const rows = preview.map(r => {
      ...
      return {
        supplier_id: supplierMap[r.supplier_name] ?? null,
    ...
    const { error } = await supabase.from('purchase_invoices').insert(rows)
    setSaving(false)
  ...
  const fieldLabel: Record<keyof Row, string> = {
    supplier_name: '중기업체명', project_name: '현장명', issue_date: '발행일',
    period_start: '기간시작', period_end: '기간종료',
    business_no: '사업자/주민번호', representative: '대표자', memo: '비고(계좌)',
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-2xl shadow-xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 shrink-0">
          <h2 className="font-bold text-gray-900">매입계산서 업로드</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {step === 'upload' && (
            <div className="text-center py-10">
              ...
              <p className="text-gray-600 mb-2 font-medium">매입계산서 엑셀 파일을 선택하세요</p>
            ...
            <div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-gray-50 text-gray-600">
                      <th className="text-left px-3 py-2">중기업체</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {preview.slice(0, 20).map((r, i) => (
                      <tr key={i}>
                        <td className="px-3 py-1.5 font-medium text-gray-900">{r.supplier_name}</td>
