...
export default function EquipmentList({ ownership }: Props) {
  ...
  async function load() {
    const [{ data: eq }, { data: sup }] = await Promise.all([
      supabase.from('equipment').select('*, supplier:suppliers(*)')
  ...
  const filtered = equipment.filter(e => {
    ...
    const matchSearch = (e.plate_no ?? '').includes(search) ||
      ...
      (e.supplier as Supplier)?.ceo_name?.includes(search)
  ...
  const title = ownership === 'own' ? '장비(자차)' : '장비(타사)'
  const statusLabel: Record<string, string> = { available: '대기', dispatched: '배차중', maintenance: '정비중' }
    ...
    <div className="p-4 md:p-8">
          ...
          <div key={e.id} className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2">
                <span className="text-xl">{e.type === 'excavator' ? <ExcavatorIcon size={22} /> : e.type === 'dump' ? '🚛' : '🚚'}</span>
                <div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-y-1.5 text-sm">
              ...
              <div className="text-gray-700">{(e.supplier as Supplier)?.name ?? '-'} {(e.supplier as Supplier)?.ceo_name ? <span className="text-gray-400 text-xs">({(e.supplier as Supplier)?.ceo_name})</span> : ''}</div>
              <div className="text-gray-400">정기검사</div>
              <div>{e.inspection_expire ? checkExpire(e.inspection_expire, 30) : <span className="text-gray-300">-</span>}</div>
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              <tr key={e.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-5 py-3 text-gray-500">{(e.supplier as Supplier)?.ceo_name ?? '-'}</td>
                <td className="px-5 py-3 text-sm">
                  {e.inspection_expire ? checkExpire(e.inspection_expire, 30) : <span className="text-gray-300">-</span>}
                </td>
                <td className="px-5 py-3 text-sm">
      ...
      {modalOpen && (
        <SupplierEquipmentModal equipment={selected} suppliers={suppliers} ownership={ownership}
          defaultSupplierId={selected?.supplier_id ?? ''}
          onClose={() => setModalOpen(false)}
          onSaved={() => { setModalOpen(false); load() }} />
      )}
      {excelOpen && (
        <EquipmentExcelUploadModal suppliers={suppliers} ownership={ownership}
